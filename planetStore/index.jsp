<%@page contentType="text/html;charset=UTF-8" language="java" import="java.sql.*,java.util.*"%>
<!DOCTYPE html>


<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
	<meta HTTP-equiv="Expires" content="0">
	<meta HTTP-equiv="kiben" content="no-cache">
    <title>星球販賣所</title>
    <link rel="stylesheet" href="css/main.css"> <!-- navBar & footer-->
    <link rel="stylesheet" href="css/index.css">
    <script src="https://kit.fontawesome.com/a076d05399.js"></script> <!--符號引用-->
	<style>
	
		/* The Modal (background) */
		#logSignModal {
			<% 
				// 帳密輸入錯誤就自動打開modal box
				String err_code = request.getParameter("error");
				if( err_code != null ) {
					if( err_code.equals("1") ) {
						//session.invalidate();
						out.print("display: block; /* Hidden by default */");
					} // if
					else {
						out.print("display: none; /* Hidden by default */");
					} // else
				}// if
				else {
					out.print("display: none; /* Hidden by default */");
				} // else
				
			%>
			
			position: fixed; /* Stay in place */
			z-index: 9; /* Sit on top */
			left: 0;
			top: 0;
			width: 100%; /* Full width */
			height: 100%; /* Full height */
			overflow: auto; /* Enable scroll if needed */
			background-color: rgb(0,0,0); /* Fallback color */
			background-color: rgba(0,0,0,0.4); /* Black w/ opacity */
		}
	</style>
</head>
<%
	// 讀取已經登入的用戶資訊
	String memID = "";
	try {
		memID = (String)session.getAttribute("id");
	} catch( Exception e ) {
		memID ="";
	}
	
	out.print(memID);
	
%>


<body>
    <div class="wrapper">
        <div class="navBar">
            <ul>
                <li><a href="index.jsp">星球販賣所
				<%
					if(memID!=null && !memID.equals(""))
					{
						out.print("Hi ! " + memID);
					} // if				
				%></a></li>
				<%
					if( memID != null && !memID.equals("") ) {
						out.print("<li><a href=\"member.jsp\"><img class=\"icon\" src='image/tianwang.jpg'></a></li>");
						//out.print("<li><a id=\"logSignModalBtn\"><img class=\"icon\" src='image/tianwang.jpg'></a></li>");
					} // if
					else {
						out.print("<li><a id=\"logSignModalBtn\"><img class=\"icon\" src='image/tianwang.jpg'></a></li>");
					} // else
				%>
                
                <li><a id="cartModalBtn"><i class="icon fas fa-shopping-cart"></i></a></li>
                <li class="search-box">
					<input class="search-txt" type="text" name="search" placeholder="Type to search">
					<a class="search-btn" href="#"><i class="icon fa fa-search"></i></a>
                </li>
                <li><a href="intro.jsp">Contact Us</a></li>
            </ul>
        </div>

        <!-- Search -->
        <div class="search-box">
            <input class="search-txt" type="text" name="" placeholder="Type to search">
            <a class=""href=""></a>
        </div>

        <!-- (Log in / Sign up) Modal -->
        <div id="logSignModal">
            <!-- Modal content -->
            <div class="form-box">
                <span id="close">&times;</span>
                <div class="button-box">
                    <div id="btn"></div>
                    <button type="button" id="toggle-btnL" class="toggle-btn" onclick="login()">Log in</button>
                    <button type="button" id="toggle-btnR" class="toggle-btn" onclick="signup()">Sign up</button>
                </div>
                <form id="login" class="input-group" method="POST" action="check.jsp">
					<%
						if( err_code != null ) {
							if( err_code.equals("1") ) {
								out.print("<p style=\"color:white;\">&nbsp&nbsp&nbsp&nbsp帳號密碼輸入錯誤!&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp請重新輸入!!<p>");
							} // if
						} // if
					%>
                    <input type="text" name="id" class="input-field" placeholder="User ID" required>
                    <input type="password" name="pwd" class="input-field" placeholder="Enter Password" required>
                    <input type="checkbox" class="check-box"><span class="check-text">Remember Password</span>
                    <button type="submit" class="submit-btn">Log in</button>
                </form>
                <form id="signup" class="input-group" method="POST" action="new_user.jsp">
                    <input type="text" name="id"  class="input-field" placeholder="User ID" required>
                    <input type="email"  name="email"  class="input-field" placeholder="Email ID" required>
                    <input type="password" name="pwd" class="input-field" placeholder="Enter Password" required>
                    <button type="submit" class="submit-btn sign-btn">Sign up</button>
                </form>
            </div>
        </div> 

        <!-- cart Modal -->
        <div id="cartModal" class="myModal-buy-cart">
            <div id="formBox" class="form-box-cart">
                <h1>購物車</h1>
                <%
					
					String cart_pID="";
					String cart_image = "";
					String cart_pName = "";
					String cart_pPrice = "";
					
					Class.forName("com.mysql.jdbc.Driver");
					String url = "jdbc:mysql://localhost/?serverTimezone=UTC";
					Connection con = DriverManager.getConnection(url,"root","1234");
					String sql="USE `Project`";
					con.createStatement().execute(sql);
					// 取得商品資訊
					if( memID != null && !memID.equals("") ) {
						sql = "SELECT * FROM `carts` WHERE `memID` ='"+ memID +"'";
						ResultSet rs = con.createStatement().executeQuery(sql);
						while( rs.next() ) {
							cart_pID = rs.getString(3);
							sql = "SELECT * FROM `product` WHERE `productID` ='"+ cart_pID +"'";
							ResultSet rs1 = con.createStatement().executeQuery(sql);
							if( rs1.next() ) {
								cart_image = rs1.getString(3);
								cart_pName = rs1.getString(2);
								cart_pPrice = rs1.getString(4);
								out.print("<div class='cartProduct'>");
								out.print("<div><img src='"+ cart_image +"' class='icon-product'></div>");
								out.print("<div><!--商品名稱--><p>"+ cart_pName +"</p></div><!--金錢-->");
								out.print("<div><p>"+ cart_pPrice +"</p></div>");
								out.print("<div><span class='del-product'>&times;</span></div></div>");
							} // if
						} // if
						
						out.print("<div class='checkout'><form><button><a href='buyhistory.jsp'>結帳</a></button> <!-- id='add' --></form></div> ");
						
					} // if
					else {
						out.print("<br><br><h3>請先登入!</h3>");
					} // else
				%>
            </div>
        </div>

        <div class="content">
            <section class="S1">
                <div class="card-carousel">
				<%
					Class.forName("com.mysql.jdbc.Driver");
					url = "jdbc:mysql://localhost/?serverTimezone=UTC";
					con = DriverManager.getConnection(url,"root","1234");
					sql="USE `Project`";
					con.createStatement().execute(sql);
					sql = "SELECT * FROM `product` ORDER BY `crate` DESC";
					ResultSet rs = con.createStatement().executeQuery(sql);
					for( int i = 0; i < 5; i++ ) {
						if( rs.next() ) {
							out.print("<div class='my-card'><img src='" + rs.getString(3) +"'></div>");
						} // if
					} // for
					
					con.close();
				%>
                    
                </div>
            </section>
            <section class="S2">
                <div class="tabs">
                    <button class="tablink L" onclick="openPage('Home', this, '#4cc9f0')">九大行星</button>
                    <button class="tablink M" onclick="openPage('News', this, 'orange')" id="defaultOpen">B612</button>
                    <button class="tablink R" onclick="openPage('Contact', this, '#b9e769')">VIRUS</button>
                </div>

                <div id="Home" class="tabcontent"> <!-- 9大行星 -->
					<%	
						int product_count = 0;
						int row_count=0;
						int c = 0;
						Class.forName("com.mysql.jdbc.Driver");
						url = "jdbc:mysql://localhost/?serverTimezone=UTC";
						con = DriverManager.getConnection(url,"root","1234");
						sql="USE `Project`";
						con.createStatement().execute(sql);
						sql = "SELECT * FROM `product` WHERE `type` = '1'";
						rs = con.createStatement().executeQuery(sql);
						rs.last();
						product_count = rs.getRow();
						row_count = (int)Math.ceil((double)product_count / 3);
						rs.first();
						out.print(row_count);
						
						for( int i = 0; i < row_count; i++ ) {
							out.print("<div class=\"row\">");
							for( int j = 0; j < 3 && 3*i+j+1 <= product_count; j++ ) {
								sql = "SELECT * FROM `product` WHERE `type` = '1' AND `number` = '" +(3*i+j+1)+ "'";
								rs = con.createStatement().executeQuery(sql);
								out.print("<div class='product'><div class='product-img'><a href='product.jsp?type=1&number=" + (i*3+j+1) +"'>");
								if( rs.next() ) {
									out.print("<img src='"+ rs.getString(3) +"'></a></div>");
									out.print("<div class='product-text'>" + rs.getString(2) + "</div>");
									out.print("<div class='product-price'> NT " + rs.getString(4) + "</div></div>");
								} // if

							} // for 
							out.print("</div>");
						} // for
						con.close();
					%>
                    
                </div>

                <div id="News" class="tabcontent"> <!--小王子-->
					<%	
						product_count = 0;
						row_count=0;
						c = 0;
						Class.forName("com.mysql.jdbc.Driver");
						url = "jdbc:mysql://localhost/?serverTimezone=UTC";
						con = DriverManager.getConnection(url,"root","1234");
						sql="USE `Project`";
						con.createStatement().execute(sql);
						sql = "SELECT * FROM `product` WHERE `type` = '2'";
						rs = con.createStatement().executeQuery(sql);
						rs.last();
						product_count = rs.getRow();
						row_count = (int)Math.ceil((double)product_count / 3);
						rs.first();
						out.print(row_count);
						
						for( int i = 0; i < row_count; i++ ) {
							out.print("<div class=\"row\">");
							for( int j = 0; j < 3 && 3*i+j+1 <= product_count; j++ ) {
								sql = "SELECT * FROM `product` WHERE `type` = '2' AND `number` = '" +(3*i+j+1)+ "'";
								rs = con.createStatement().executeQuery(sql);
								out.print("<div class='product'><div class='product-img'><a href='product.jsp?type=2&number=" + (i*3+j+1) +"'>");
								if( rs.next() ) {
									out.print("<img src='"+ rs.getString(3) +"'></a></div>");
									out.print("<div class='product-text'>" + rs.getString(2) + "</div>");
									out.print("<div class='product-price'> NT " + rs.getString(4) + "</div></div>");
								} // if

							} // for 
							out.print("</div>");
						} // for
						con.close();
					%>
                    
                </div>

                <div id="Contact" class="tabcontent"> <!--病毒系列-->
                    <%	
						product_count = 0;
						row_count=0;
						c = 0;
						Class.forName("com.mysql.jdbc.Driver");
						url = "jdbc:mysql://localhost/?serverTimezone=UTC";
						con = DriverManager.getConnection(url,"root","1234");
						sql="USE `Project`";
						con.createStatement().execute(sql);
						sql = "SELECT * FROM `product` WHERE `type` = '3'";
						rs = con.createStatement().executeQuery(sql);
						rs.last();
						product_count = rs.getRow();
						row_count = (int)Math.ceil((double)product_count / 3);
						rs.first();
						out.print(row_count);
						
						for( int i = 0; i < row_count; i++ ) {
							out.print("<div class=\"row\">");
							for( int j = 0; j < 3 && 3*i+j+1 <= product_count; j++ ) {
								sql = "SELECT * FROM `product` WHERE `type` = '3' AND `number` = '" +(3*i+j+1)+ "'";
								rs = con.createStatement().executeQuery(sql);
								out.print("<div class='product'><div class='product-img'><a href='product.jsp?type=3&number=" + (i*3+j+1) +"'>");
								if( rs.next() ) {
									out.print("<img src='"+ rs.getString(3) +"'></a></div>");
									out.print("<div class='product-text'>" + rs.getString(2) + "</div>");
									out.print("<div class='product-price'> NT " + rs.getString(4) + "</div></div>");
								} // if

							} // for 
							out.print("</div>");
						} // for
						con.close();
					%>
                </div>

                <div id="About" class="tabcontent"> <!--生日風景-->
                    <div class="product">
                        <div class="product-img"><a href="product.jsp"><img src="image/tianwang.jpg"></a></div>
                        <div class="product-text">天王星</div>
                        <div class="product-price">NT99</div>
                    </div>
                </div>
            </section>
        </div>
		<%
			
			Class.forName("com.mysql.jdbc.Driver");
			url = "jdbc:mysql://localhost/?serverTimezone=UTC";
			con = DriverManager.getConnection(url,"root","1234");
			sql="USE `Project`";
			con.createStatement().execute(sql);
			sql = "SELECT * FROM `counter` WHERE `counterID` = '1'";
			
			rs = con.createStatement().executeQuery(sql);
			int counts = 0;
			
			if( rs.next() ) {
				
				counts = Integer.parseInt(rs.getString(2));
				out.print(counts);
				
				if( session.isNew() ) {
					counts++;
				} // if
				
				sql = "UPDATE `counter` SET `count` ='" + counts + "' WHERE `counterID` = '1'";
				con.createStatement().executeUpdate(sql);
				con.close();
				
			} // if
			
		%>
        <footer class="footer">
            <span>Copyright © 2021 All Rights Reserved<br>
                  Tel：0800-091-000<br>
                  訪客人數: <%=counts%>
            </span>
        </footer>
    </div>

    <script src="js/tab.js"></script>
	<%
		if( memID != null && !memID.equals("") ) {
			out.print("<script src=\"js/modal_LogSign1.js\"></script> <!-- modal box的控制 -->");
		} // if
		else {
			out.print("<script src=\"js/modal_LogSign.js\"></script> <!-- modal box的控制 -->");
		} // else
	%>
    
    <script src="js/LogSign.js"></script> <!-- 專為login/signup的modal box -->
    <script src="https://code.jquery.com/jquery-3.3.1.min.js" 
        integrity="sha384-tsQFqpEReu7ZLhBV2VZlAu7zcOV+rXbYlF2cqB8txI/8aZajjp4Bqd+V6D5IgvKT" 
        crossorigin="anonymous">
    </script> <!-- for carousel -->
    <script src="js/carousel.js"></script>
</body>
</html>